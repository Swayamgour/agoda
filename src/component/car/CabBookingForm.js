          ...
          <InsideFrom>
            <InputRow>
              ...
              {tripType !== 'hourly' && (
                <InputGroup>
                  <InputLabel>
                    <FiMapPin size={16} /> Drop Location
                  </InputLabel>
                  <LocationInput>
                    <InputField
                      type='text'
            ...
            <InputRow>
              <InputGroup>
                <InputLabel>
                  <FiCalendar size={16} /> Pickup Date
                </InputLabel>
                <DateInput>
                  <DateDisplay
            ...
            {tripType === 'round-trip' && (
              <InputRow>
                <InputGroup>
                  <InputLabel>
                    <FiCalendar size={16} /> Return Date
                  </InputLabel>
                  <DateInput>
                    <DateDisplay>
                      {returnDate
            ...
            {/* <InputRow>
                  ...
                  {openPassengerPicker && (
                    <Dropdown>
                      {passengerOptions.map(num => (
                        <DropdownItem
                          key={num}
                          className={passengers === num ? 'selected' : ''}
                          onClick={() => {
                            setPassengers(num)
                            setOpenPassengerPicker(false)
                          }}
              ...
              <InputGroup>
                <InputLabel>
                  <FaCar size={16} /> Cab Type
                </InputLabel>
                <SelectInput>
                  <SelectDisplay
                    onClick={() => setOpenCabTypePicker(!openCabTypePicker)}
                  >
                    <span>
                      {cabTypes.find(type => type.value === cabType)?.label}
                    </span>
                    <FiChevronDown />
                  </SelectDisplay>
                  {openCabTypePicker && (
                    <Dropdown>
                      {cabTypes.map(type => (
                        <DropdownItem
                          key={type.value}
                          className={cabType === type.value ? 'selected' : ''}
                          onClick={() => {
                            setCabType(type.value)
                            setOpenCabTypePicker(false)
                          }}
                        >
                          <span>
            ...
            {tripType === 'hourly' && (
              <InputGroup>
                <InputLabel>
                  <FiClock size={16} /> Rental Duration
                </InputLabel>
                <SelectInput>
                  <SelectDisplay
                    onClick={() =>
                      setOpenRentalHoursPicker(!openRentalHoursPicker)
                    }
                  >
                    <span>
                      {
                        hourOptions.find(opt => opt.value === rentalHours)
                          ?.label
                      }
                    </span>
                    <FiChevronDown />
                    ...
                    <Dropdown>
                      {hourOptions.map(opt => (
                        <DropdownItem
                          key={opt.value}
                          className={
                            rentalHours === opt.value ? 'selected' : ''
                          }
                          onClick={() => {
